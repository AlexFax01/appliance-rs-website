...
- ZIP matching is advisory, not an availability promise. `src/content/coverage.ts` includes the source/license/date for all 19 towns. “Service Pulse” layers lightweight coverage zones and five key markers over the verified Google map; the complete 19-town directory opens the same city/ZIP coverage card. Visitors can switch to the unobstructed interactive map. The iframe is created only when this below-the-fold section approaches the viewport, so it does not block the initial page load.
