...
- ZIP matching is advisory, not an availability promise. `src/content/coverage.ts` includes the source/license/date for all 19 towns. Service Areas presents the 33 KB AVIF (53 KB WebP fallback) 3D map on the left and the verified Google map on the right. The Google iframe is created only when this below-the-fold section approaches the viewport, so it does not block the initial page load.
