...
test('photo preparation, limit and prepared SMS retention',async({page})=>{
 ...
 const sms=await page.locator('#sms-ready-link').getAttribute('href');expect(sms).toMatch(/^sms:\+18649244349[?&]body=/);expect(decodeURIComponent(sms!)).toContain('Name: Controlled QA');expect(decodeURIComponent(sms!)).toContain('I selected 2 photos');
});
